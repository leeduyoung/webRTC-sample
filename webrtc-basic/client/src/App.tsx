...
function App() {
  ...
  useEffect(() => {
    ...
    socketObj.on('all_users', (allUsers: Array<{ id: string; email: string }>) => {
    ...
    socketObj.on('getCandidate', (candidate: RTCIceCandidateInit) => {
      peerConnectionObj.addIceCandidate(new RTCIceCandidate(candidate)).then(() => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        socketObj.emit('join_room', { room: '1234', email: 'sample@naver.com' })
      })
      .catch((error) => {
    ...
    const createAnswer = (sdp: RTCSessionDescription) => {
      peerConnectionObj.setRemoteDescription(new RTCSessionDescription(sdp)).then(() => {
        console.log('answer set remote description success')
        peerConnectionObj
          .createAnswer({ offerToReceiveVideo: true, offerToReceiveAudio: true })
          .then((sdp1) => {
            console.log('create answer')
            peerConnectionObj.setLocalDescription(new RTCSessionDescription(sdp1))
